...
enum RestStatus {
  ...
  /// prepares a response or preconnect to an origin from which the page will
  /// need resources.
  earlyHints(103),
  ...
  /// An Nginx HTTP server extension. This code is introduced to log the case
  /// when the connection is closed by client while HTTP server is processing
  ...
  /// A 599 status code is typically encountered when the server encounters an
  /// unexpected error or crash.
  /// Some common causes of a 599 status code include:
  /// Server overload: When the server receives more requests than it can
  ///   handle, it may become overloaded and crash, resulting in a 599 status
  ///   code.
  /// Network failures: If there is a disruption in the network connection
  /// between the server and the client, it can lead to a 599 error.
  /// Server-side errors: This could be caused by a software bug, a
  /// misconfiguration of the server, or a problem with the code running on the
  /// server.
  networkConnectTimeoutError(599);

  /// Contains the int value of the http code.
  final int value;

  /// Constructor
  const RestStatus(this.value);

  /// Given a int value of a http code, returns the correspondent enum value.
  static RestStatus? fromValue([int? value]) {
    return RestStatus.values.firstWhereOrNull(
